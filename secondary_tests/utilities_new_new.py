...
def prepare_data(test_size=0.2, accept_data_filename="l1calo_hist_EGZ_extended.root", reject_data_filename="l1calo_hist_ZMUMU_extended.root",
                 data_subdir="ZMUMU_EGZ_extended_np_pd", format_mode="SuperCell_ET", get_pT=True, distance_boundaries=[0.1,0.2,0.3,0.4], equalised=False,rate_estimation=False,max_entries=None):
    ...
    if os.path.exists(os.path.join(save_path, "np_data.npz")) and os.path.exists(os.path.join(save_path, "input_df.parquet")):
        ...
        if equalised:
            DFs = equalise(DFs)
        print(DFs[0].shape, DFs[0].columns, DFs[1].shape, DFs[1].columns)
        ...
        rejected_df = pd.DataFrame({'offline_ele_pt': DFs[1]['offline_ele_pt'], 'EventNumber': DFs[1]['EventNumber'], 'Label': 0})

        input_np = format_numpy_training_input(DFs, format_mode, distance_boundaries, rate_estimation, max_entries)

        if not max_entries == None:
            accepted_labels = accepted_labels[:max_entries]
            rejected_labels = rejected_labels[:max_entries]
            accepted_df = accepted_df[:max_entries]
            rejected_df = rejected_df[:max_entries]
...
def format_numpy_training_input(DFs,format_mode, distance_boundaries, rate_estimation, max_entries):
    ...
    if not max_entries == None:
        accepted_numpy = accepted_numpy[:max_entries]
        rejected_numpy = rejected_numpy[:max_entries]
...
def train_evaluate_all_classifiers(binary_classifiers, X_train, X_test, y_train, y_test, pd_passthrough_train, pd_passthrough_test, description, data_subdir, ZB_X_test=None, ZB_pd_passthrough_test=None):
    results = []
    ...
    for name, Classifier in tqdm(binary_classifiers.items()):
        ...
        if not results_found:
            ...
            try:
                ...
                try:
                    if (not (type(ZB_X_test) is type(None))) and (not (type(ZB_pd_passthrough_test) is type(None))):
                    ...
                    print(f"Failed rate computation for {name}: {e}")
                    rate = "NULL"

                # try:
                #     if (not ZB_X_test == None) and (not ZB_pd_passthrough_test == None):
                #         ZB_y_pred = clf.predict(ZB_X_test)
                #         ZB_pd_passthrough_test["pred"] = ZB_y_pred
                #         print(ZB_pd_passthrough_test.columns)
                #         event_accept_count = count_accepted(ZB_pd_passthrough_test)
                ...
                result_entry.update({key: "NULL" for key in ["Accuracy", "TN", "FP", "FN", "TP", "Recall", "Precision", "F1", "MSE","EventCount", "Rate"]})
            ...
            try:
                ...
                result_entry.update({"Accuracy": accuracy, "TN": tn, "FP": fp, "FN": fn, "TP": tp, "Recall": recall, "Precision": precision, "F1": f1, "MSE": mse,"EventCount":event_accept_count, "Rate":rate})
            except Exception as e:
                print(f"Failed evaluation metrics for {name}: {e}")
                result_entry.update({"Accuracy": "NULL", "TN": "NULL", "FP": "NULL", "FN": "NULL", "TP": "NULL", "Recall": "NULL", "Precision": "NULL", "F1": "NULL", "MSE": "NULL","EventCount":"NULL", "Rate": "NULL"})
...
def get_classifiers(data_subdirs):
    # Find the CSV file ending with "pareto_optimal.csv" in the given subdirectory
    csv_files = glob.glob(os.path.join(data_subdirs, '*pareto_optimal.csv'))
    
    if not csv_files:
        raise FileNotFoundError("No file ending with 'pareto_optimal.csv' found in the subdirectory.")

    # Load the first matching CSV file
    df = pd.read_csv(csv_files[0])

    # Extract classifier names from the first column
    classifiers = df.iloc[:, 0].tolist()
...
def save_csv(description, results, data_subdir):
    ...
    with open(output_file, mode="w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=["Classifier", "Accuracy", "TN", "FP", "FN", "TP", "Precision", "Recall", "F1", "MSE","EventCount", "Rate"])
        writer.writeheader()
